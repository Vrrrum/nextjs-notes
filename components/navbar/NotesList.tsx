...
const NotesList: React.FC<Props> = ({notes, setNotes, selectedNoteId, setSelectedNoteId}) => {
    const handleDeleteNote = (noteId: string) => {
        ...
        const newNotes: Note[] = notes.filter(note => note.id !== noteId);
            ...
            {notes.map((note: Note) => (
                <div key={note.id} onClick={() => setSelectedNoteId(note.id)} className={`${selectedNoteId != null && selectedNoteId === note.id ? `bg-neutral-600` : 'bg-neutral-700'} p-2 rounded-lg hover:bg-neutral-600 cursor-pointer`}>
                    <div className="flex flex-row justify-between">
                        ...
                        <div className="flex justify-center w-10 hover:cursor-pointer text-red-600 hover:text-red-500"
                             onClick={(e) => {
                            e.stopPropagation();
